...
int main()
{
    ...
    Employee* emp3 = employee_new();
    Employee* emp4 = employee_new();
    Employee* emp5 = employee_new();
    emp1->id = 1000;
    strcpy(emp1->nombre, "Ana");
    emp1->horasTrabajadas = 11;
    emp1->sueldo = 10000;

    emp2->id = 1001;
    strcpy(emp2->nombre, "Claudia");
    emp2->horasTrabajadas = 21;
    emp2->sueldo = 20000;

    emp3->id = 1002;
    strcpy(emp3->nombre, "Maria");
    emp3->horasTrabajadas = 31;
    emp3->sueldo = 30000;

    emp4->id = 1003;
    strcpy(emp4->nombre, "Pedro");
    emp4->horasTrabajadas = 41;
    emp4->sueldo = 40000;

    emp5->id = 1004;
    strcpy(emp5->nombre, "Jose");
    emp5->horasTrabajadas = 51;
    emp5->sueldo = 50000;
    ...
    //escribirTexto("listaEmpleados.csv", lista);
...
/*
    ...
    while(!feof(f))//con feof recorres hasta que encuentre el final del archivo, son los datos de un empleado
    {
        ...
        if(cant == 4)//mostras si devolvio 4 variables en fscanf
        {
            ...
            auxEmpleado = employee_newParametros(buffer[0], buffer[1], buffer[2], buffer[3]);//ahora ya estan convertidos entoces le paso los buffer sin el atoi o el atof
    ...
    /* if(lista != NULL)
     {
         ...
         printf("Lista creada exitosamente con ll_newLinkedList().\n\n");
     }
    ...
    if(empPush != NULL)
    {
        ...
        printf("\n\nUtilice el ll_push() para insertar un elemento en el indice 3, muestro los empleados antes del Push y despues.\n\n");
...
void leerTexto(char* path, LinkedList* lista)
{
    ...
    //f = fopen("numeros.txt", "r");//lo abris como lectura de texto
    //f = fopen("empleados.csv", "r");//lo abris como lectura de texto
    f = fopen(path, "r");//lo creamos en file3, lo creamos como csv texto y lo leo con uno que lee csv
    if(f == NULL)
    {
        printf("No se pudo abrir el archivo\n");
        exit(EXIT_FAILURE);
...
{
    int rta = 0;
    Employee* e = (Employee*) elemento;//casteas el elemento a lo que quieras filtrar
